	...
	if x > 0 && y > 0 {
		...
		for i := 1; i <= y; i++ {
			...
			if i == 1 {
				...
				for j := 1; j <= x; j++ {
					// If the current column is the first column, print a '/' character.
					if j == 1 {
						z01.PrintRune('/')
						// If the current column is the last column, print a '\' character.
					} else if j == x {
						z01.PrintRune('\\')
				...
				// Loop through each column of the figure.
				for j := 1; j <= x; j++ {
					// If the current column is the first or the last column, print a '*' character.
					if j == x || j == 1 {
						z01.PrintRune('*')
